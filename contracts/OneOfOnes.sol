...
contract OneOfOnes is ERC721A, Ownable {
  /// When frozen, the owner cannot change the metadata anymore
  bool frozen = false;
  ...
  function mint(string memory name, string memory description, string memory image, address recipient) external onlyOwner {
    _metadata[currentIndex] = new Metadata(name, description, image);
      ...
      abi.encodePacked(
        ...
        Base64.encode(
          bytes(
            ...
            abi.encodePacked('{"name":"', metadata.name, '", "description":"', metadata.description, '", "image": "', image, '"}')
          )
        )
      )
    );
